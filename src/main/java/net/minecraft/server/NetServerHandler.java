...
import org.bukkit.event.player.PlayerGuildeAskEvent;
import org.bukkit.event.player.PlayerInteractEntityEvent;
import org.bukkit.event.player.PlayerItemHeldEvent;
import org.bukkit.event.player.PlayerJobAskEvent;
...
public class NetServerHandler extends NetHandler {
    ...
    public void disconnect(String s) {
        if (!this.disconnected) {
            // CraftBukkit start
            String leaveMessage = "\u00A7e" + this.player.name + " se d\u00E9connecte.";
    ...
    public void a(Packet133Money packet) {
	    double money = packet.money;
	    PlayerMoneyAskEvent event = new PlayerMoneyAskEvent((float)packet.money, getPlayer());
	    this.server.getPluginManager().callEvent(event);
	  ...
	  public void a(Packet135Guilde packet) {
	    String guilde = packet.guilde;
	    String guildeRank = packet.guildeRank;
	    PlayerGuildeAskEvent event = new PlayerGuildeAskEvent(guilde, packet.guildeRank, getPlayer());
	    this.server.getPluginManager().callEvent(event);
	    if (guilde != event.getGuilde()) {
	      packet.guilde = event.getGuilde();
	      if (guildeRank != event.getGuildeRank()) {
	        packet.guildeRank = event.getGuildeRank();
	        sendPacket(packet);
	      }
	    }
	  }

	  public void a(Packet134Job packet) {
	    String job = packet.job;
	    int jobLevel = packet.jobLevel;
	    int jobExperience = packet.jobExperience;
	    int jobMaxExperience = packet.jobMaxExperience;
	    PlayerJobAskEvent event = new PlayerJobAskEvent(job, jobLevel, jobExperience, jobMaxExperience, getPlayer());
	    this.server.getPluginManager().callEvent(event);
	    if (job != event.getJob())
	    {
	      packet.job = event.getJob();
	      if (jobLevel != event.getJobLevel()) {
	        packet.jobLevel = event.getJobLevel();
	      }
	      if (jobExperience != event.getJobExperience()) {
	        packet.jobExperience = event.getJobExperience();
	      }
	      if (jobMaxExperience != event.getJobMaxExperience()) {
	        packet.jobMaxExperience = event.getJobMaxExperience();
	      }
	      sendPacket(packet);
	    }
	  }
